...
export const skip: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (!connection) {
      console.log('[LOG] Invoked bot/showignore without bot being connected to any channel');
      await interaction.reply({
    ...
    if (connection.joinConfig.channelId !== (interaction.member as GuildMember).voice.channelId) {
      console.log('[LOG] Invoked bot/skip but bot and user are on different channels');
      await interaction.reply({
