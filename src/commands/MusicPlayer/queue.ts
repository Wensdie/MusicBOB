...
export const queue: Command = {
  data: new SlashCommandBuilder().setName('queue').setDescription('Showing queue.'),
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (musicPlayerQueue) {
      ...
      const queuePlayingNow = songNow ? `Playing: ${songNow.name} - ${songNow.length}` : '';
      let queueList = '';
      console.log('[LOG] Invoked bot/queue when music is played and/or queue is not empty.');
      if (musicPlayerQueue.length > 0) {
        queueList += '\nQueue:\n';
