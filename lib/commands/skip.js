        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (bot.services.MusicPlayer) {
                if (bot.services.MusicPlayer.getConnection().joinConfig.channelId !== (interaciton.member).voice.channelId) {
                    yield interaciton.reply({ content: "You cannot skip song on the other channel.", ephemeral: true });
                    return;
                }
                if (bot.services.MusicPlayer.getPlayer().state.status === AudioPlayerStatus.Idle) {
                    yield interaciton.reply({ content: "Nothing is playing right now.", ephemeral: true });
                    return;
                }
                if (bot.services.MusicPlayer.getPlayer().state.status === AudioPlayerStatus.Playing) {
                    bot.services.MusicPlayer.skipSong();
                    yield interaciton.reply({ content: "Song skiped." });
                    return;
                }
            }
        });
